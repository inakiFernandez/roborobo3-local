...
SocialBehaviorController::SocialBehaviorController( RobotWorldModel *wm )
{
    _wm = wm;
    ...
    _currentSigma = SocialBehaviorSharedData::gSigmaRef;
    ...
    _isListening = true;
    _notListeningDelay = SocialBehaviorSharedData::gNotListeningStateDelay;
    _listeningDelay = SocialBehaviorSharedData::gListeningStateDelay;
...
SocialBehaviorController::~SocialBehaviorController()
{
    _parameters.clear();
    delete nn;
    nn = NULL;
...
void SocialBehaviorController::reset()
{
    _parameters.clear();
    _parameters = _genome;
}
...
void SocialBehaviorController::step()
{
    _iteration++;
...
void SocialBehaviorController::stepBehaviour()
{
    ...
    if ( SocialBehaviorSharedData::gEnergyRequestOutput )
    {
...
void SocialBehaviorController::createNN()
{
    if ( nn != NULL ) // useless: delete will anyway check if nn is NULL or not.
        delete nn;
    
    switch ( SocialBehaviorSharedData::gControllerType )
    {
        case 0:
        {
            // MLP
        ...
        default: // default: no controller
            std::cerr << "[ERROR] gController type unknown (value: " << SocialBehaviorSharedData::gControllerType << ").\n";
            exit(-1);
    };
}


unsigned int SocialBehaviorController::computeRequiredNumberOfWeights()
{
...
void SocialBehaviorController::stepEvolution()
{
    ...
    if( gWorld->getIterations() > 0 && gWorld->getIterations() % SocialBehaviorSharedData::gEvaluationTime == 0 )
                    //dynamic_cast<SocialBehaviorWorldObserver*>(gWorld->getWorldObserver())->getGenerationItCount() == 0 )  //todelete
    {
        loadNewGenome();
        _nbGenomeTransmission = 0;
    }
    ...
    if ( SocialBehaviorSharedData::gLogGenome && gWorld->getIterations() % ( SocialBehaviorSharedData::gEvaluationTime * SocialBehaviorSharedData::gSnapshotsFrequency ) == 1 )
    {
        // Logging: full genome
        std::string sLog = std::string("");
...
void SocialBehaviorController::selectRandomGenome()
{
    if(_genomesList.size() != 0)
    {
        ...
        // modified medea
        if ( SocialBehaviorSharedData::gIndividualMutationRate > rand()/RAND_MAX )
        {
            switch ( SocialBehaviorSharedData::gMutationOperator )
            {
                case 0:
                    mutateUniform();
                    break;
                case 1:
                    ...
                    break;
                case 2:
                    mutate(SocialBehaviorSharedData::gSigma);
                    break;
            }
        }
        else
...
void SocialBehaviorController::selectFirstGenome()
{
    if(_genomesList.size() != 0)
    {
...
bool SocialBehaviorController::storeGenome(std::vector<double> genome, int senderId, int senderBirthdate, float sigma)
{
...
void SocialBehaviorController::mutate( float sigma) // mutate within bounds.
{
    _genome.clear();
...
void SocialBehaviorController::mutateUniform() // mutate within bounds.
{
    _genome.clear();
...
void SocialBehaviorController::resetRobot()
{
    _nbInputs = 0;
    ...
    _nbHiddenLayers = SocialBehaviorSharedData::gNbHiddenLayers;
    
    _nbNeuronsPerHiddenLayer = new std::vector<unsigned int>(_nbHiddenLayers);
    for(unsigned int i = 0; i < _nbHiddenLayers; i++)
        (*_nbNeuronsPerHiddenLayer)[i] = SocialBehaviorSharedData::gNbNeuronsPerHiddenLayer;
    ...
    {
        _genome.push_back((double)(rand()%SocialBehaviorSharedData::gNeuronWeightRange)/(SocialBehaviorSharedData::gNeuronWeightRange/2)-1.0); // weights: random init between -1 and +1
    }
...
void SocialBehaviorController::broadcastGenome()
{
    // remarque \todo: limiting genome transmission is sensitive to sensor order. (but: assume ok)
    
    for( int i = 0 ; i < _wm->_cameraSensorsNb && ( SocialBehaviorSharedData::gLimitGenomeTransmission == false || ( SocialBehaviorSharedData::gLimitGenomeTransmission == true && _nbGenomeTransmission < SocialBehaviorSharedData::gMaxNbGenomeTransmission ) ); i++)
    {
        ...
        if ( targetIndex >= gRobotIndexStartOffset && targetIndex < gRobotIndexStartOffset+gNumberOfRobots )   // sensor ray bumped into a robot : communication is possible
        {
            ...
            SocialBehaviorController* targetRobotController = dynamic_cast<SocialBehaviorController*>(gWorld->getRobot(targetIndex)->getController());
            ...
            if ( targetRobotController->isListening() )
            {
                ...
                if ( dice <= SocialBehaviorSharedData::gProbaMutation )
                {
                    dice = float(rand() %100) / 100.0;
                    if ( dice < 0.5 )
                    {
                        sigmaSendValue = _currentSigma * ( 1 + SocialBehaviorSharedData::gUpdateSigmaStep ); // increase sigma
                        
                        if (sigmaSendValue > SocialBehaviorSharedData::gSigmaMax)
                        {
                            sigmaSendValue = SocialBehaviorSharedData::gSigmaMax;
                        }
                    }
                    else
                    {
                        sigmaSendValue = _currentSigma * ( 1 - SocialBehaviorSharedData::gUpdateSigmaStep ); // decrease sigma
                        
                        if ( sigmaSendValue < SocialBehaviorSharedData::gSigmaMin )
                        {
                            sigmaSendValue = SocialBehaviorSharedData::gSigmaMin;
                        }
                    }
...
void SocialBehaviorController::loadNewGenome()
{
    if ( _wm->isAlive() || gEnergyRefill )  // ( gEnergyRefill == true ) enables revive
    {
        ...
        if (_genomesList.size() > 0)
        {
            ...
            switch ( SocialBehaviorSharedData::gSelectionMethod )
            {
                case 0:
                    selectRandomGenome();
                    break;
                case 1:
                    ...
                    break;
                default:
                    std::cerr << "[ERROR] unknown selection method (gSelectionMethod = " << SocialBehaviorSharedData::gSelectionMethod << ")\n";
                    exit(-1);
            }
        ...
        else
        {
            ...
            if ( _wm->isAlive() == true )
            {
                ...
                if ( SocialBehaviorSharedData::gNotListeningStateDelay != 0 ) // ie. -1 (infinite,dead) or >0 (temporary,mute)
                {
                    _isListening = false;
...
void SocialBehaviorController::logCurrentState()
{
    // Logging
