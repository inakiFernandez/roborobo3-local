...
if __name__ == "__main__":
    ...
    fig = plt.figure("Cooperative Foraging",figsize=(16, 12),dpi=70,facecolor=bgcolor)
    ...
    for i in range(len(sys.argv) - excessParam):
        dat = []
        print(i)
        colors = [(1.0, 0.50, 0.0, float(i // 2 )/5.0),(0.25, 0.0, 1.0, float(i // 2 )/5.0)]
        #print(colors)
        if i // 2 < 1:
        ...
        #color = colors[i // 2] + (0.9 if ((i%2)==0) else 0.3,)
        color = colors[i % 2]
        print(color)
        ...
        handlesPlots.append(plot_one_curve(dat, color, axis, r''+name ,True))#sys.argv[excessParam + i], True)  #names[i], True)
    ...
    print(sys.argv[excessParam ] + "testAll.png")
    plt.tight_layout()
    savefig(sys.argv[excessParam] + "testAll.png", dpi=100 )
    #plt.show()
