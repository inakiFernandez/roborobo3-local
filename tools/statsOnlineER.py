...
if __name__ == "__main__":
    ...
    listfnames = read_fnames(sys.argv[1])
    #dat = read_logfile(sys.argv[1])
    #dat2 = read_logfile(sys.argv[2])
    data = []
    for name in listfnames:
        data.append(read_logfile(name))
    #maxFit = np.max(dat + dat2)
    #maxFit = np.max(dat2)

    maxFit = np.max(data)

    targetFit = float(sys.argv[3]) * maxFit
    lastEpoch = len(data[0])
    print lastEpoch
    print len(data)
    print len(data[0][0])
    backEpoch = int(np.floor(float(sys.argv[2]) * lastEpoch))
    stats = []
    for exp in data:
        avgCum = []
        fixedBudgetFit = []
        ...
        statstmp = [avgCum, fixedBudgetFit, timeToTarget, accOverTgt]
        stats.append(statstmp)

    nb_graphs = len(statstmp)
    ...
    labs = ["medea","brait"] #linspace(1.0, 0.0, 5)
    for i in xrange(nb_graphs):
        plot_boxplot(zip(*stats)[i], colors[1:], axis[i], labs,
                     sig=False)

    axis[2].set_ylim([-2, lastEpoch+2])
